...
if usrOS != "Windows":
    scmd = {'ls':'ls','q':'exit','cd':'cd','mkdir':'mkdir','rmdir':'rmdir','execpython':'python','touch':'touch','os':'ckos','echo':'echo'}
...
while exitpysh == False:
    ...
    cwd=cwd.replace(home, "", 1)
    cmdf = False
    cpi = "{USR}@{usrOS}:{cdm}$ ".format(usrOS=usrOS.lower().replace("darwin", "macos"), USR=USR.lower(), cdm=cdm)
    cmd = input(cpi)
    fullcmd = cmd
    cmd = cmd.split()[0]
